package com.amina.Client.security;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication().withUser("amina").password(passwordEncoder.encode("123")).roles("AGENT", "USER");
	 ...
	 protected void configure(HttpSecurity http) throws Exception {
		 ...
		 http.authorizeRequests().antMatchers("/saveClient").hasAnyRole("ADMIN","AGENT");
		 http.authorizeRequests().antMatchers("/ListeClient")
		 .hasAnyRole("ADMIN","AGENT","USER");

		 http.authorizeRequests()
		 .antMatchers("/supprimerClient","/modifierClient","/updateClient")
		 .hasAnyRole("ADMIN");
